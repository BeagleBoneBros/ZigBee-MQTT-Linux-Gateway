 ...
 *
 */
//#define ZDP_MgmtBindReq( dstAddr, StartIndex, SecurityEnable ) \ ZDP_SendData( &ZDP_TransID, dstAddr, Mgmt_Bind_req, 1, &StartIndex, SecurityEnable)
...
/*
 * ZDP_AppCnfStartCommissioningReq - Send a App Config BDB Start Commissioning Req
 *
 * @MT SPI_CMD_APP_CNF_START_COMMISSIONING_REQ
 * (byte commissioningMode)
 *
 */
extern afStatus_t ZDP_AppCnfStartCommissioningReq( byte commissioningMode );

/*
 * ZDP_AppCnfAddInstallCodeReq - Send a App Config BDB Add InstallCode Req
 *
 * @MT SPI_CMD_APP_CNF_ADD_INSTALL_CODE_REQ
 * (InstallCode_t installcode)
 *
 */
extern afStatus_t ZDP_AppCnfAddInstallCodeReq(  uint8 *ieeeAddress, uint8 length, uint8 *installCode, uint16 crc );

/*
 * ZDP_ZclActionReq - Send a ZCL NV Action Req
 *
 * @MT 
 * ()
 *
 */
extern afStatus_t ZDP_ZclActionReq( uint16 cmdID, uint16 id, uint16 ndx, uint16 len, void *pData );

/*
 * ZDP_MgmtBindReq - Send a Mgmt Bind Request to a node on the network
 *
 * @MT SPI_CMD_MGMT_BIND_REQ
 * (zAddrType_t *dstAddr, byte startCommissioning)
 *
 */
extern afStatus_t ZDP_MgmtBindReq( uint16 dstAddr, byte startIndex, byte SecurityEnable );

/*
 * ZDP_AppCnfSetChannelReq - Send a App Config BDB Set Channel Req
 *
 * @MT SPI_CMD_APP_CNF_SET_CHANNEL_REQ
 * (bool is Primary)
 * (uint32 channelMask)
 *
 */

extern afStatus_t ZDP_AppCnfSetChannelReq( bool isPrimary, uint32 channelMask );

/*
 * ZDP_AppCnfBdbSetTCRequireKeyExchange - Send a App_Config_Set_TC_Require_Key_Exchange_Req
 *
 * @MT SPI_CMD_APP_CNF_BDB_SET_TC_REQUIRE_KEY_EXCHANGE
 * (bool requireTCLK)
 *
 */

extern afStatus_t ZDP_AppCnfSetTCRequireKeyExchangeReq( bool requireTCLK );

/*
 * ZDP_AppCnfBDBSetJoinUsesICKeyReq - Send a App_Config_Set_Join_Uses_InstallCodeKey_Req
 *
 * @MT SPI_CMD_APP_CNF_BDB_SET_JOINUSESINSTALLCODEKEY
 * (bool usesIC)
 *
 */

extern afStatus_t ZDP_AppCnfSetJoinUsesICKeyReq( bool usesIC );

/*
 * ZDP_AppCnfBDBSetRemoteEndDeviceTimeoutReq - Send a App_Config_Set_Remote_EndDevice_Timeout_Req
 *
 * @MT SPI_CMD_APP_CNF_SET_DEFAULT_REMOTE_ENDDEVICE_TIMEOUT
 * (uint8 timeout)
 *
 */

extern afStatus_t ZDP_AppCnfSetRemoteEndDeviceTimeoutReq( uint8 edTimeout );
...
extern afStatus_t SYS_SetDevNwkInfoReq( uint8 len, uint8 *buf );
extern afStatus_t SYS_GpAddressConflict( uint8 len, uint8 *buf );
