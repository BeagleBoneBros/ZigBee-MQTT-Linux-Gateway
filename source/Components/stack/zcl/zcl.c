...
zclMSPWCommandsCb_t zclMSPWCommandsCb = NULL;
...
zclProcMsgStatus_t zcl_ProcessMessageMSG( afIncomingMSGPacket_t *pkt )
{
  ...
  if ( !interPanMsg && zcl_ProfileCmd( inMsg.hdr.fc.type ) )
  {
    ...
    {
      if (zclMSPWCommandsCb != NULL)
      {
        status = zclMSPWCommandsCb(&inMsg);
      }
      else
      {
...
static uint8 zclProcessInReadCmd( zclIncoming_t *pInMsg )
{
  ...
  uint16 len;
  uint8 i;
  //uint8 proxyTableStatus;
  //uint8 proxyTable = 0;
  //uint8 proxyTableLen = 0;
  //uint8 *proxyTableBuffer = NULL;
  ...
  for ( i = 0; i < readCmd->numAttr; i++ )
  {
	...
	if ( zclFindAttrRec( pInMsg->msg->endPoint, pInMsg->msg->clusterId,
                         readCmd->attrID[i], &attrRec ) ) //If the request is for any other recorded attribute
    {
  ...
  /*if( proxyTableBuffer != NULL )
  {
  	zcl_mem_free( proxyTableBuffer );
  }*/
