/**************************************************************************************************
  Filename:       zcl.h
  Revised:        $Date: 2015-11-16 21:11:21 -0800 (Mon, 16 Nov 2015) $
  Revision:       $Revision: 44546 $
 ...
 * CONSTANTS
 */
//Finding and Bindng endpoint type
#define BDB_FINDING_AND_BINDING_INITIATOR                  0x01
#define BDB_FINDING_AND_BINDING_TARGET                     0x02
#define BDB_FINDING_AND_BINDING_NO_EP_FOUND                0x03
#define BDB_FINDING_AND_BINDING_NO_IDENTIFY                0x04
...
typedef ZStatus_t (* zclMSPWCommandsCb_t)(zclIncoming_t * pInMsg);
...
extern zclMSPWCommandsCb_t zclMSPWCommandsCb;
...
#define zcl_SendWriteNoRsp(a,b,c,d,e,f,g) (zcl_SendWriteRequest( (a), (b), (c), (d), ZCL_CMD_WRITE_NO_RSP, (e), (f), (g) ))
  ...
  #define zcl_build_uint16   osal_build_uint16
  ...
  extern int zcl_memcmp( const void *dst, const void *src, unsigned int len );
  extern void zcl_mem_free(void *ptr);
  extern uint8* zcl_msg_allocate( uint16 len );
  extern uint8 zcl_msg_deallocate( uint8 *msg_ptr );
  ...
  extern uint16 zcl_build_uint16( uint8 *swapped );
  extern uint32 zcl_build_uint32( uint8 *swapped, uint8 len );
  extern void *zcl_cpyExtAddr(uint8 * pDest, const uint8 * pSrc);

#endif

extern uint8 zclReadGetProxyEntry( uint16 nvId, uint8* pData, uint8* len );
extern void debugPrint( uint8 label );
