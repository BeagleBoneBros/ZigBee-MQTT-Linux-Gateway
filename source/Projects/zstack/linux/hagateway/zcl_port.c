 ...
 */
extern uint8 pt_ZclReadGetProxyEntry( uint16 nvId, uint8* pData, uint8* len );
...
uint8 zclReadGetProxyEntry( uint16 nvId, uint8* pData, uint8* len )
{
	return pt_ZclReadGetProxyEntry( nvId, pData, len );
}

void debugPrint( uint8 label )
{
	uiPrintfEx(trERROR, "Debug Label %d", label);
}
...
int zcl_memcmp( const void *dst, const void *src, unsigned int len )
{
...
{
  return zcl_memcpy( (void *)pDest, (void *)pSrc, 8 );
}
...
uint8* zcl_msg_allocate( uint16 len )
{
  osal_msg_hdr_t *hdr;
  ...
  hdr = (osal_msg_hdr_t *) zcl_mem_alloc(
      (short) (len + sizeof(osal_msg_hdr_t)) );
  if ( hdr )
  {
    hdr->next = NULL;
    hdr->len = len;
...
uint8 zcl_msg_deallocate( uint8 *msg_ptr )
{
  uint8 *x;
...
uint16 zcl_build_uint16( uint8 *swapped )
{
 ...
 * @fn zcl_nv_actions
 *
 * @brief: This function will be in charge of sending a request to perform an NV action 
 *         via protobuffer.
 *
 * @param action - This flag should be 0 for item init, 1 for read, 2 for write
 * @param id     - Register ID with which the action will be performed
 * @param len    - Length of the buffer
 * @param *buf   - Pointer to the buffer containing the payload for the message
 *
 * @return uint8 ZCL Status
 */
uint8 zcl_nv_actions( uint8 action, uint16 id, uint16 index, uint16 len, void *buf )
{
  ZStatus_t status = FAILURE;
  uint8 *pBuf;
  ZclActionReq zclActReq = ZCL_ACTION_REQ__INIT;
  uint8 rspcmdid;
  uint8 *pRsp;  
  uint16 rsplen;

  uiPrintfEx(trDEBUG,  "Sending ZCL Action Request - action:%d, id:%X, bufLen:%d\n",
  				 action, id, len ); 
  
  zclActReq.action = action;
  zclActReq.id = id;
  zclActReq.index = index;
  zclActReq.payload.len = len;
  zclActReq.payload.data = buf;
  
  len = zcl_action_req__get_packed_size( &zclActReq );
  pBuf = malloc( len );
  if ( pBuf )
  {
    zcl_action_req__pack( &zclActReq, pBuf );
  
    // Send the API client message
    //status = (afStatus_t)sendAPICExpectDefaultStatus( ZSTACK_CMD_IDS__ZCL_ACTION_REQ, len, pBuf );

	// send request to NPI synchronously
    pRsp = apicSendSynchData( GW_ZSTACK_HANDLE, ZSTACK_SYS_IDS__RPC_SYS_PROTOBUF, 
                              ZSTACK_CMD_IDS__ZCL_ACTION_REQ, len, pBuf,
                              NULL, &rspcmdid, &rsplen );
	
	if ( pRsp )
	{
	  if ( (rspcmdid == ZSTACK_CMD_IDS__ZCL_ACTION_RSP) && (rsplen > 0) )
	  {
		ZclActionRsp *pActionRsp;
		pActionRsp = zcl_action_rsp__unpack( NULL, rsplen, pRsp );
		
		if ( pActionRsp )
		{				 
		  if(action == ZCL_NV_READ)
		  {
		  	memcpy(buf, pActionRsp->payload.data, pActionRsp->payload.len);
		  }
		  status = pActionRsp->index;
		  zcl_action_rsp__free_unpacked( pActionRsp, NULL );
		}
		
	  }
	  else
	  {
		uiPrintfEx(trERROR, "Expected GwZclActionRsp, got %d\n", rspcmdid );
	  }
...
//note that this function must not be called again until the processing that depend on a previous call is finished, since it modifies a global (which in turn also points to a static)
endPointDesc_t *afFindEndPointDesc( uint8 EndPoint )
{
  int i;
  static SimpleDescriptionFormat_t simpleDesc;
...
static ZStatus_t zclGeneral_ProcessInGroupsClient( zclIncoming_t *pInMsg )
{
  ...
  switch ( pInMsg->hdr.commandID )
  {
    ...
    case COMMAND_GROUP_REMOVE_RSP:
      ...
      status = gwGroupsClusterIndCB( &rsp, pInMsg->msg->endPoint );
      break;
...
static ZStatus_t zclGeneral_ProcessInScenesClient( zclIncoming_t *pInMsg )
{
  ...
  switch ( pInMsg->hdr.commandID )
  {
    ...
    case COMMAND_SCENE_STORE_RSP:
      ...
      status = gwScenesClusterIndCB( &rsp, pInMsg->msg->endPoint );
      break;
...
static ZStatus_t zclGeneral_ProcessInAlarmsClient( zclIncoming_t *pInMsg )
{
  ...
  switch ( pInMsg->hdr.commandID )
  {
    ...
    case COMMAND_ALARMS_CLEAR:
	  ...
	  stat = gwAlarmsClusterIndCB( &alarm, pInMsg->msg->endPoint );
      break;
...
ZStatus_t zclPollControl_HdlIncoming( zclIncoming_t *pInMsg )
{
  ...
  if ( (pInMsg->hdr.commandID == COMMAND_POLL_CONTROL_CHECK_IN) &&
       zcl_ClientCmd( pInMsg->hdr.fc.direction ) )
  {
    ...
    stat = gwZclPollControlCheckInCB( &cmd, pInMsg->msg->endPoint );
  }
