...
static uint8 sendNPIExpectDefaultStatusAPPCNF( int cmdID, int len, uint8 *pData );
static uint8 sendNPIExpectDefaultStatusOsalNvAction( int cmdID, uint16 id, uint16 ndx, uint16 len, void *pData );
...
/**************************************************************************************************
 *
 * @fn          sendNPIExpectDefaultStatusAPPCNF
 *
 * @brief       Send a request message and expect the normal "default" response (MacDefaultRsp)
 *
 * @param       cmdID - messages command ID (cmd1 according to Z-Stack Monitor and Test API doc)
 * @param       len - length of pData buffer to send
 * @param       pData - pointer to buffer to send
 *
 * @return      synchronous return status
...
static uint8 sendNPIExpectDefaultStatusAPPCNF( int cmdID, int len, uint8 *pData )
{
  return (sendNPIExpectDefaultStatusZNP( MT_RPC_SYS_APP_CNF, cmdID, len, pData ));
}

/**************************************************************************************************
 *
 * @fn          sendNPIExpectDefaultStatusOsalNvAction
 *
 * @brief       Send a request message and expect the normal "default" response (MacDefaultRsp)
 *
 * @param       cmdID - messages command ID (cmd1 according to Z-Stack Monitor and Test API doc)
...
static uint8 sendNPIExpectDefaultStatusOsalNvAction( int cmdID, uint16 id, uint16 ndx, uint16 len, void *pData )
{
  if (cmdID == 0)
  {
	...
	return (osal_nv_item_init( id, len, pData ));
  }
  else if (cmdID == 1)
  {
  
    return (osal_nv_read( id, ndx, len, pData ));
  }
  else if (cmdID == 2)
  {
  
    return (osal_nv_write( id, ndx, len, pData));
  }
  return (ZFailure);
}
...
/*********************************************************************
 * @fn          ZDP_AppCnfStartCommissioningReq
 *
 * @brief       This builds and sends a ZDP_AppCnfStartCommissioningReq message.
 *
 * @param       commissioningMode - Commissioning Mode
 *
 * @return      afStatus_t
 */
afStatus_t ZDP_AppCnfStartCommissioningReq( byte commissioningMode )
{
  ZStatus_t status;
  uint8 buf[1];
  ...
  switch (commissioningMode)
  {
	case 2:
	case 4:
  	  buf[0] = commissioningMode;
  	  status = sendNPIExpectDefaultStatusAPPCNF( MT_APP_CNF_START_COMMISSIONING, 1, buf );
		break;
	default:
      break;
  }
...
/*********************************************************************
 * @fn          ZDP_AppCnfAddInstallCodeReq
 *
 * @brief       This builds and sends a ZDP_AppCnfAddInstallCodeReq message.
 *
 ...
 * @return      afStatus_t
 */
afStatus_t ZDP_AppCnfAddInstallCodeReq(  uint8 *ieeeAddress, uint8 length, uint8 *installCode, uint16 crc )
{
  ZStatus_t status;
  uint8 buf[27];
  buf[0] = 1;

  memcpy( &buf[1], ieeeAddress, 8 );
  memcpy( &buf[9], installCode, length);

  buf[25] = LO_UINT16(crc);
  buf[26] = HI_UINT16(crc);

  status = sendNPIExpectDefaultStatusAPPCNF( MT_APP_CNF_ADD_INSTALLCODE, 27, buf );
 ...
 * @fn          ZDP_ZclActionReq
 *
 * @brief       This builds and sends a ZDP_ZclActionReq message.
 *
 * @param       installCode - InstallCode
 *
 * @param		CRC - crc 
 ...
 * @return      afStatus_t
 */
afStatus_t ZDP_ZclActionReq( uint16 cmdID, uint16 id, uint16 ndx, uint16 len, void *pData )
{
  ZStatus_t status;
  
  status = sendNPIExpectDefaultStatusOsalNvAction( cmdID, id, ndx, len, pData );

  return (status);
}

/*********************************************************************
 * @fn          ZDP_AppCnfBdbSetChannel
 *
 * @brief       This builds and sends a App_Config_Set_Channel_Req message.
 *
 * @param       isPrimary - Primary channel set if TRUE or Secondary channel set if FALSE
 * 				channelMask - bit mask to select the channel for the ZNP
 *
 * @return      afStatus_t
 */
afStatus_t ZDP_AppCnfSetChannelReq( bool isPrimary , uint32 channelMask )
{
  ZStatus_t status;
  uint8 buf[5];

  buf[0] = isPrimary; //TRUE isPrimary or FALSE for secondary 
  buf[1] = BREAK_UINT32( channelMask , 0);
  buf[2] = BREAK_UINT32( channelMask , 1);
  buf[3] = BREAK_UINT32( channelMask , 2);
  buf[4] = BREAK_UINT32( channelMask , 3);
  
  status = sendNPIExpectDefaultStatusAPPCNF( MT_APP_CNF_SET_CHANNEL, 5, buf );
 ...
 * @return      afStatus_t
 ********************************************************************/
afStatus_t ZDP_MgmtBindReq(uint16 dstAddr, byte startIndex, byte securityEnabled)
{
  ZStatus_t status;
  uint8 buf[3];
  ...
  status = sendNPIExpectDefaultStatusZDO( MT_ZDO_MGMT_BIND_REQ, 3, buf );

  return (status);
}

/*********************************************************************
 * @fn          ZDP_AppCnfBdbSetTCRequireKeyExchange
 *
 * @brief       This builds and send a App_Config_Set_TC_Require_Key_Exchange_Req message.
 *
 * @param       requireTCLK - Require TCLK exchange (Bool) TRUE = Require is mandatory, 
 * 				FALSE = Require is not mandatory
 *
 * @return      afStatus_t
 */
afStatus_t ZDP_AppCnfSetTCRequireKeyExchangeReq( bool requireTCLK )
{
  ZStatus_t status;
  uint8 buf[1];
  ...
  status = sendNPIExpectDefaultStatusAPPCNF( MT_APP_CNF_SET_TC_REQUIRE_KEY_EXCHANGE, 1, buf );

  return (status);
}/*********************************************************************
 * @fn          ZDP_APP_CNF_BDB_SET_JOINUSESINSTALLCODEKEY
 *
 * @brief       This builds and send a App_Cnf_BDB_Set_JoinUsesInstallCodeKey_Req message.
 *
 * @param       usesIC - Use IC TRUE = Use of IC is mandatory, 
 * 				FALSE = Use of IC is not mandatory
 *
 * @return      afStatus_t
 */
afStatus_t ZDP_AppCnfSetJoinUsesICKeyReq( bool usesIC )
{
  ZStatus_t status;
  uint8 buf[1];
  ...
  status = sendNPIExpectDefaultStatusAPPCNF( MT_APP_CNF_SET_JOINUSESINSTALLCODEKEY, 1, buf );

  return (status);
}

/*********************************************************************
 * @fn          APP_CNF_SET_REMOTE_ENDDEVICE _TIMEOUT
 *
 * @brief       This builds and send a App_Cnf_BDB_Set_RemoteEndDeviceTimeout_Req message.
 *
 * @param       usesIC - Use IC TRUE = Use of IC is mandatory, 
 * 				FALSE = Use of IC is not mandatory
 *
 * @return      afStatus_t
 */
afStatus_t ZDP_AppCnfSetRemoteEndDeviceTimeoutReq( uint8 edTimeout )
{
  ZStatus_t status;
  uint8 buf[1];
  ...
  status = sendNPIExpectDefaultStatusAPPCNF( MT_APP_CNF_SET_ENDDEVICETIMEOUT, 1, buf );
...
uint8 ZDApp_StartJoiningCycle( void )
{
  //ZStatus_t status;
  uint8 buf[1];
  buf[0] = NETWORK_FORMATION;
	
  //sendNPIZDO( MT_ZDO_STARTUP_FROM_APP, 0, NULL );
  
  sendNPIExpectDefaultStatusAPPCNF( MT_APP_CNF_START_COMMISSIONING , 1 , buf);
 ...
 * @return      afStatus_t
 */
afStatus_t SYS_SetDevNwkInfoReq( uint8 len, uint8 *buf )
{
  afStatus_t status;

  status = sendNPIExpectDefaultStatusZNP( MT_RPC_SYS_UTIL, MT_UTIL_SET_DEV_NWK_INFO, len, buf );
  
  return (status);
}

/*********************************************************************
 * @fn          SYS_GpAddressConflict
 *
 * @brief       This builds and sends a ZDP_AppCnfAddInstallCodeReq message.
 *
 * @param       len - Length of the buffer
 ...
 * @return      afStatus_t
 */
afStatus_t SYS_GpAddressConflict( uint8 len, uint8 *buf )
{
  afStatus_t status;

  status = sendNPIExpectDefaultStatusZNP(MT_RPC_SYS_GP, MT_GP_ADDRESS_CONFLICT, len, buf );
