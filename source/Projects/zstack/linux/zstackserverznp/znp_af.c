 ...
 * @fn      sendGpSecRsp
 *
 * @brief   Send security response to ZNP
 *
 * @param   pGpSecRspMsg - pointer to the Security Response Message.
 *
 * @return  afStatus
 */
void sendGpSecRsp( GpSecRspMsg *pGpSecRspMsg )
{
  afStatus_t status;
  //const uint8 len = sizeof( gp_SecRsp_t ) + 4;
  const uint8 len = 38;
  ...
  if ( pMsg )
  {
    ...
    *pBuf++ = (uint8)pGpSecRspMsg->status;
    *pBuf++ = (uint8)(pGpSecRspMsg->dgpstubhandle);
    *pBuf++ = (uint8)(pGpSecRspMsg->appid);
    memcpy(pBuf, &pGpSecRspMsg->srcid, 4);
    pBuf += 4;
    memcpy(pBuf, &pGpSecRspMsg->gpdextaddr, 8);
    pBuf += 8;
    *pBuf++ = (uint8)(pGpSecRspMsg->endpoint);
    *pBuf++ = (uint8)(pGpSecRspMsg->gpdfseclvl);
    *pBuf++ = (uint8)(pGpSecRspMsg->gpdfkeytype);
    memcpy(pBuf, pGpSecRspMsg->gpdkey.data, 16);
    pBuf += 16;
    memcpy(pBuf, &pGpSecRspMsg->gpdsecframecounter, 4);

    status = sendNPIExpectDefaultStatusZNP( MT_RPC_SYS_GP, MT_GP_SEC_RSP, len, pMsg );
