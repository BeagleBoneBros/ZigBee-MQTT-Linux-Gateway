...
uint8 zgDefaultTCRequireKeyExchange = BDB_DEFAULT_TC_REQUIRE_KEY_EXCHANGE;
uint8 zgDefaultJoinUsesICKey = BDB_DEFAULT_JOIN_USES_INSTALL_CODE_KEY;
uint8 zgDefaultRemoteEndDeviceTimeout = BDB_DEFAULT_REMOTE_ENDDEVICE_TIMEOUT;

extern apicHandle_t zmainClientHandle; // ZStack API client handle
extern bool zstack_mutex_valid;        // Check for zstack mutex
...
extern void *APPCNFCommissioningNotiCB( void *pStr );
...
static void processAPPCNFCommissioningNoti( uint16 len, uint8 *pMsg );
static uint8 sendNPIExpectDefaultStatusSYS( int cmdID, int len, uint8 *pData );
static void processGPSecReq( uint8 *pMsg );
static void processGPDataInd( uint16 len, uint8 *pMsg );
                      ...
                      uint16 len, uint8 *pMsg )
{
  ...
  if ( subSys == MT_RPC_SYS_APP_CNF ) //Catching Incoming Asynchronous messages from the APP Config Sub Sys (0x4F)
  {
    switch ( cmdID )
    {
      //APP Config BDB Commissioning Notification is an Indication type message that comes as a response from a Start Commissioning Req
	  case MT_APP_CNF_COMMISSIONING_NOTIFICATION:
	  	{
		  /* This command should only have a length equal to 3, since the payload has 1byte corresponding to Status, 
		   * 1byte for Commissioning Mode and 1byte for Remaining Commissioning Modes.
		   * *Note: the Remaining Commissioning Modes aren't currently implemented on the ZNP.
		   */
		  processAPPCNFCommissioningNoti( len, pMsg );
	  	}
        break;
		
	  default:
		  uiPrintfEx(trDEBUG, "znp_misc handleAsyncMsgs: Unrecognized APP CNF cmdID:%x\n", cmdID );
		break;
    }
  }
  else if ( subSys == MT_RPC_SYS_ZDO )
  {
    switch ( cmdID )
    {
  ...
  else if (subSys == MT_RPC_SYS_GP )
  {
    switch ( cmdID )
    {
    case MT_GP_SEC_REQ:
        processGPSecReq( pMsg );
      break;

    case MT_GP_DATA_IND:
        processGPDataInd( len, pMsg );
      break;

    case MT_GP_DATA_CNF:
        //processGPDataConfirmInd( len, pMsg );
      break;
    }
  }
  else
 ...
 * @fn          processGPSecReq
 *
 * @brief       processes incoming Green Power Security Request
 *
 * @param       pMsg - pointer to message
 *
 * @return      none
...
static void processGPSecReq( uint8 *pMsg )
{
  ...
  pBuf = (gp_SecReq_t *) osal_msg_allocate( sizeof(gp_SecReq_t) );
  if ( pBuf )
  {
    pBuf->hdr.event = GP_INCOMING_SEC_RSP_CMD;
    pBuf->hdr.status = 0;

    pBuf->gpd_ID.AppID = *pMsg++;

    // GPD ID
    if( pBuf->gpd_ID.AppID == GP_APP_ID_DEFAULT )
    {
      // Copy the Src ID
      pVal = (uint8_t*)&pBuf->gpd_ID.GPDId.SrcID;
      UINT32_TO_BUF_LITTLE_ENDIAN(pVal, *(uint32_t*)pMsg);
      // To skip the IEEE
      // SrcID + IEEE = 4 + 8
      pMsg += 12;
    }
    // IEEE ID
    else if( pBuf->gpd_ID.AppID == GP_APP_ID_GP )
    {
      // Copy the GPD IEEE address
      pMsg += 4;
      osal_cpyExtAddr(pBuf->gpd_ID.GPDId.GPDExtAddr, pMsg);
      pMsg += 8;
    }

    pBuf->EndPoint = *pMsg++;
    pBuf->gp_SecData.GPDFSecLvl = *pMsg++;
    pBuf->gp_SecData.GPDFKeyType = *pMsg++;
    pVal = (uint8_t*)&pBuf->gp_SecData.GPDSecFrameCounter;
    UINT32_TO_BUF_LITTLE_ENDIAN(pVal, *(uint32_t*)pMsg);
    // To skip the GPDSecFrameCounter
    pMsg += 4;

    pBuf->dGPStubHandle = *pMsg++;
 ...
 * @fn          processGPDataInd
 *
 * @brief       processes incoming Green Power Data Indication
 *
 * @param       len - length of message
 * @param       pMsg - pointer to message
 *
...
static void processGPDataInd( uint16 len, uint8 *pMsg )
{
  ...
  pBuf = (gp_DataInd_t *) osal_msg_allocate( sizeof(gp_DataInd_t) + asduLen );
  memset(pBuf, 0, sizeof(gp_DataInd_t) + asduLen);

  if ( pBuf )
  {
    pBuf->hdr.event = GP_INCOMING_DATA_IND_CMD;
    pBuf->hdr.status = 0;
    ...
    pBuf->SeqNumber = *pMsg++;
    pBuf->srcAddr.addrMode = *pMsg++;
    pBuf->srcPanID = BUILD_UINT16( pMsg[0], pMsg[1] );
    pMsg += 2;

    osal_cpyExtAddr(pBuf->srcAddr.addr.extAddr, pMsg);
    pMsg += 8;

    pBuf->appID = *pMsg++;
    pBuf->GPDFSecLvl = *pMsg++;
    pBuf->GPDFKeyType = *pMsg++;
    pBuf->AutoCommissioning = *pMsg++;
    pBuf->RxAfterTx = *pMsg++;
    pBuf->SrcId = BUILD_UINT32( pMsg[0], pMsg[1], pMsg[2], pMsg[3] );
    pMsg += 4;

    pBuf->EndPoint = *pMsg++;
    pBuf->GPDSecFrameCounter = BUILD_UINT32( pMsg[0], pMsg[1], pMsg[2], pMsg[3] );
    pMsg += 4;
    pBuf->GPDCmmdID = *pMsg++;
    pBuf->GPDasduLength = *pMsg++;

    if(pBuf->GPDasduLength > 0)
    {
      memcpy (pBuf->GPDasdu, pMsg, pBuf->GPDasduLength );
      pMsg += pBuf->GPDasduLength;
    }
    pBuf->MIC = BUILD_UINT32( pMsg[0], pMsg[1], pMsg[2], pMsg[3] );
...
/**************************************************************************************************
 *
 * @fn          processAPPCNFCommissioningNotification
 *
 * @brief       processes incoming ZNP APP CNF Commissioning Notification Indication
 *
 * @params      len   - LENGTH of the payload. This should always equal 3 according to document 
 * 					    "Z-Stack Monitor and Test API".
 * 				*pMsg - POINTER to the incoming message payload. Includes: status, commissioning mode
 * 						and remaining commissioning mode.
 *
 * @return      none
 *
 **************************************************************************************************/
static void processAPPCNFCommissioningNoti( uint16 len, uint8 *pMsg )
{
  ...
  // Do not proceed if the length is not the expected.
  if (len == 3){ 

	AppCnfCommNotiPacket_t *pBuf; // Buffer structure to hold parameters of incoming message

    pBuf = ( AppCnfCommNotiPacket_t *)osal_msg_allocate( sizeof(AppCnfCommNotiPacket_t) );
    if ( pBuf ){
	  // Load buffer with the data from the incoming message.
	  pBuf->hdr.event = APP_CNF_COMM_NOTIFICATION;
	  pBuf->status = *pMsg++;
	  pBuf->commissioningMode= *pMsg++;
	  pBuf->remainingCommMode= *pMsg;
	  // Send osal message with the parameters received 
	  if ( osal_msg_send( zspbTaskID, (uint8 *) pBuf ) != SUCCESS ){
	    // Error, message wasn't sent
	    osal_msg_deallocate( (uint8 *) pBuf );
	  }
...
uint8 osal_nv_item_init( uint16 id, uint16 len, void *buf )
{
  ...
  pBuf = malloc(sizeof(id) + sizeof(len) + len);
  if ( pBuf )
  {
    uint8 *pPtr = pBuf;
    *pPtr++ = LO_UINT16( id );
    ...
    *pPtr++ = HI_UINT16( len );
    ...
    status = sendNPIExpectDefaultStatusSYS( MT_SYS_OSAL_NV_ITEM_INIT, len,
        pBuf );
 ...
 * @fn      osal_nv_delete
 *
 * @brief   Converts function call to ZNP OSAL NV Delete Message
 *
 * @param   id  - Valid NV item Id.
 * @param   len - Length of data to write.
 *
...
uint8 osal_nv_delete( uint16 id, uint16 len )
{
  ...
  if ( pBuf )
  {
    ...
    *pPtr++ = HI_UINT16( len );
    ...
    status = sendNPIExpectDefaultStatusSYS( MT_SYS_OSAL_NV_DELETE, (4),
        pBuf );
...
void znpReset( uint8 mode, uint8 nvReset, uint8 shutdown )
{
  ...
  if ( nvReset )
  {
    uint8 nvStartup = (ZCD_STARTOPT_DEFAULT_NETWORK_STATE | ZCD_STARTOPT_DEFAULT_CONFIG_STATE);
 ...
 *
 */
void GetDevNwkInfo( nwkIB_t* pNIB )
{
  uint8 *pBuf, rspcmdid;

  // send request to NPI synchronously
  pBuf = apicSendSynchData( ZNP_API_CLIENT, MT_RPC_SYS_UTIL, MT_UTIL_GET_DEV_NWK_INFO,
      0, NULL, NULL, &rspcmdid, NULL );

  if ( rspcmdid == MT_UTIL_GET_DEV_NWK_INFO )
  {
    pNIB->nwkDevAddress = BUILD_UINT16( pBuf[0], pBuf[1] );
    pBuf += 2;
    pNIB->nwkPanId = BUILD_UINT16( pBuf[0], pBuf[1] );
    pBuf += 2;
    pNIB->SequenceNum = *pBuf++;
