...
pthread_mutex_t zstack_mutex;
pthread_mutexattr_t zstack_mutex_attr;
bool zstack_mutex_valid = FALSE;
...
apisSysParams_t sysParams;
  ...
  configTableItem_t configItems[] =
  {
   ...
   { &zgDefaultTCRequireKeyExchange, "BDB_DEFAULT_TC_REQUIRE_KEY_EXCHANGE", TYPE_UINT8, 1 },
   { &zgDefaultJoinUsesICKey, "BDB_DEFAULT_JOIN_USES_INSTALL_CODE_KEY", TYPE_UINT8, 1 },
   { &zgDefaultRemoteEndDeviceTimeout, "BDB_DEFAULT_REMOTE_ENDDEVICE_TIMEOUT", TYPE_UINT8, 1 },
...
{
  if (register_pb_pointer_list(&zstack_pb_pointer_list) == NULL)
  {
    uiPrintfEx(trERROR, "failed registering protobuf pointer lists\n");
  }
  
  if (pthread_mutexattr_init(&zstack_mutex_attr) != 0)
  {
    uiPrintfEx(trFATAL, "appMain> Could not successfully create mutex.\n");
    return (NULL);
  }
  else if ((pthread_mutexattr_settype(&zstack_mutex_attr, PTHREAD_MUTEX_RECURSIVE) != 0) ||
    (pthread_mutex_init(&zstack_mutex, &zstack_mutex_attr) != 0))
  {
    pthread_mutexattr_destroy(&zstack_mutex_attr);
    uiPrintfEx(trFATAL, "appMain> Could not successfully create mutex.\n");
    return NULL;
  }
  else
  { 
    zstack_mutex_valid = TRUE;
  }
...
int appInitPhase3( apicHandle_t *handles )
{
  ...
  uiPrintfEx(trUNMASKABLE, "\n");
  uiPrintfEx(trUNMASKABLE, " ************************************************\n");
  uiPrintfEx(trUNMASKABLE, " *  ZStack Server for ZNP v1.0.2d               *\n");
  uiPrintfEx(trUNMASKABLE, " ************************************************\n");
  uiPrintfEx(trUNMASKABLE, "\n");
  ...
  apicInitializeConnectionInfp(zmainClientHandle, NPI_SERVER_NAME, NPI_LAYER_NUMBER);

  if (!(zstack_mutex_valid))
  {
    //Don't proceed if mutex couldn't be created
    uiPrintfEx(trFATAL, "appMain> Could not successfully create mutex. "
    "Exiting...\n");
    return 254;
  }
