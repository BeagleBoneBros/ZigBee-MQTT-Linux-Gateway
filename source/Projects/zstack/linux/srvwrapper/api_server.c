...
static int apisSendData( uint16 len, uint8 *pData, int connection )
{
  ...
  if ( connection < 0 )
  {
    ...
    while ( entry )
    {
      if ( entry->sock != listenSocketHndl )
      {
        ...
        visualization_args.remoteLN = get_connection_layer_number(entry->sock);
        visualization_args.remoteName = get_connection_name(entry->sock);

        trace_print_buf(pData, len, "[bcst] ", &visualization_args);
        print_pb_msg(pData[2], pData[3], pData + 4, pData[0] + (pData[1] >> 8), 0, "", FALSE, &visualization_args, sizeof(apic16BitLenMsgHdr_t), "[bcst] ");
  ...
  else
  {
    ...
    if ( reserveActiveConnection( connection ) == 0 )
    {
      ...
      trace_print_buf(pData, len, "[ucst] ", &visualization_args);
      print_pb_msg(pData[2], pData[3], pData + 4, pData[0] + (pData[1] >> 8), 0, "", FALSE, &visualization_args, sizeof(apic16BitLenMsgHdr_t), "[ucst] ");
        ...
        if ( c == listenSocketHndl )
        {
          ...
          else
          {
            ...
            else
            {
			  ...
			  uiPrintfEx(trCONNECTION, "Connected to new client, connection # %d\n", justConnected);
            }
          }
        }
        else
          ...
          else
          {
            ...
            switch ( apisErrorCode )
            {
              case APIS_ERROR_IPC_RECV_DATA_DISCONNECT:
                uiPrintfEx(trCONNECTION, "Disconnected from client %s (connection # %d)\n", get_connection_name(c), c);
...
// Receive only NPI header first. Then then number of bytes indicated by length.
  ...
  else if ( n == sizeof(apisHdrBuf) )
  {
    ...
    if ( ret == APIS_LNX_SUCCESS )
    {
      if ( len != 0xFFFF )
      {
        if (apisHdrBuf.subSys == (RPC_DUMMYSYS_CONNECTION_INFO | RPC_CMD_AREQ))
        {
          if (apisHdrBuf.cmdId == RPC_CMD_CONNTCTION_ID)
          {
            set_connection_details(connection, (char *)buf + 1, len - 1, buf[0]);

            uiPrintfEx(trCONNECTION, "Client at connection # %d identified itself as %.*s\n", connection, len - 1, buf + 1);
          }
        }

        STRING_START(trace_prefix,"[read] %02X:%02X:%02X:%02X%s", apisHdrBuf.lenL, apisHdrBuf.lenH, apisHdrBuf.subSys, apisHdrBuf.cmdId, len > 0 ? ":" : "");

        if (apisHdrBuf.subSys != RPC_DUMMYSYS_CONNECTION_INFO)
        {
          visualization_args.localLN = pAPIS_SysParams->layerNum;
          visualization_args.localName = pAPIS_SysParams->serverName;
          visualization_args.remoteLN = get_connection_layer_number(connection);
          ...
          visualization_args.max_layers = INVALID_LAYER_NUMBER;
          trace_print_buf(buf, len, trace_prefix, &visualization_args);
          print_pb_msg(apisHdrBuf.subSys, apisHdrBuf.cmdId, buf, len, 0, "", FALSE , &visualization_args, sizeof(apic16BitLenMsgHdr_t), "[read] ");
        ...
        {
          trace_print_buf(buf, len, trace_prefix, NULL);
        }
      }
      else
      {
  ...
  {
    uiPrintfEx(trERROR, "apisConnectionHandle(): partial reception of buffer header\n" );
  }
