...
void si_unregister_idle_callback(void)
{
	...
	dev_get_gw_endpoint_info_request();
}
...
int si_send_packet(pkt_buf_t * pkt, confirmation_processing_cb_t _confirmation_processing_cb, void * _confirmation_processing_arg)
{
	...
	visualization_args.localLN = APP_LAYER_NUMNER;
	visualization_args.localName = SAMPLE_APP_NAME;
	visualization_args.remoteLN = server->layerNumber;
	visualization_args.remoteName = server->name;
	visualization_args.directionSend = true;
	visualization_args.max_layers = INVALID_LAYER_NUMBER;

	//trace_print_buf(pkt, pkt->header.len + sizeof(pkt->header), "[send] ", &visualization_args);
	
	//Remove this 2 conditions if the identify timer spam on the server log and ui console aren't an issue.
	if(!(pkt->header.cmd_id==GW_CMD_ID_T__GW_WRITE_DEVICE_ATTRIBUTE_REQ))
	{
		if(!(pkt->header.cmd_id==GW_CMD_ID_T__GW_WRITE_DEVICE_ATTRIBUTE_RSP_IND))
		{
			print_pb_msg(pkt->header.subsystem, pkt->header.cmd_id, pkt->packed_protobuf_packet, pkt->header.len, 0, "", false, &visualization_args, sizeof(pkt_buf_hdr_t), NULL);	
		}
	}
...
void si_nwk_manager_incoming_data_handler(pkt_buf_t * pkt, int len)
{
	switch (pkt->header.cmd_id)
	{
		...
		case NWK_MGR_CMD_ID_T__NWK_GET_GW_ENDPOINT_INFO_CNF:
		case NWK_MGR_CMD_ID_T__NWK_START_COMMISSIONING_CNF:
		...
		case NWK_MGR_CMD_ID_T__NWK_GET_BINDING_TABLE_RSP_IND:
			comm_device_binding_table_request_rsp_ind(pkt);
			break;
		case NWK_MGR_CMD_ID_T__NWK_SEND_SIMPLE_DESCRIPTOR_RSP_IND:
			dev_simple_descriptor_response_handler(pkt);
			break;
		default:
...
void si_gateway_incoming_data_handler(pkt_buf_t * pkt, int len)
{
	switch (pkt->header.cmd_id)
	{
		case GW_CMD_ID_T__DEV_PROCESS_IDENTIFY_QUERY_RSP_IND:
			dev_identify_query_response_handler(pkt);
			break;
		case GW_CMD_ID_T__GW_ATTRIBUTE_CHANGE_IND:
			attr_change_ind_handler(pkt);
			break;
		case GW_CMD_ID_T__GW_WRITE_DEVICE_ATTRIBUTE_RSP_IND:
			attr_write_response_indication_handler(pkt);
			break;
		...
		case GW_CMD_ID_T__DEV_ZONE_ENROLLMENT_REQ_IND:
			snsr_process_zone_enrollment_req(pkt);
			break;
		case GW_CMD_ID_T__DEV_ZONE_STATUS_CHANGE_IND:
			snsr_process_zone_status_change_ind(pkt);
			break;
		default:
...
int si_init(char * nwk_manager_server_hostname, u_short nwk_manager_server_port, char * gateway_server_hostname, u_short gateway_server_port, char * ota_server_hostname, u_short ota_server_port)
{
	...
	if ((register_pb_pointer_list(&nwkmgr_pb_pointer_list) == NULL)
		|| (register_pb_pointer_list(&gateway_pb_pointer_list) == NULL)
		|| (register_pb_pointer_list(&otasrvr_pb_pointer_list) == NULL))
	{
		fprintf(stderr,"ERROR, failed registering protobuf pointer lists\n");
		return -1;
	}

	if (tcp_new_server_connection(&network_manager_server, nwk_manager_server_hostname, nwk_manager_server_port, (server_incoming_data_handler_t)si_nwk_manager_incoming_data_handler, NWKMGR_SERVER_NAME, NWKMGR_LAYER_NUMBER, nwk_mgr_server_connected_disconnected_handler) == -1)
	{
		fprintf(stderr,"ERROR, wrong network manager server\n");
		return -1;
	}
	
	if (tcp_new_server_connection(&gateway_server, gateway_server_hostname, gateway_server_port, (server_incoming_data_handler_t)si_gateway_incoming_data_handler, GATEWAY_SERVER_NAME, GATEWAY_LAYER_NUMBER, gateway_server_connected_disconnected_handler) == -1)
	{
	...
	if (tcp_new_server_connection(&ota_server, ota_server_hostname, ota_server_port, (server_incoming_data_handler_t)si_ota_incoming_data_handler, OTASRVR_SERVER_NAME, OTASRVR_LAYER_NUMNER, ota_server_connected_disconnected_handler) == -1)
	{
...
void si_compose_address(zb_addr_t * addr,	GwAddressStructT * dstaddr)
{
	...
	dstaddr->gwendpointid = ui_get_selected_local_endpoint_id();
}
