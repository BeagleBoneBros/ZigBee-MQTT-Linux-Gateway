...
/* Process AppCnfCommissioningNotification*/
static void processNwkCommissioningNoti( AppCnfCommissioningNotification *pCommNotification);
...
{
  if ((register_pb_pointer_list(&otasrvr_pb_pointer_list) == NULL) ||
...
int appInitPhase3( apicHandle_t *handles )
{
    ...
    uiPrintfEx(trUNMASKABLE, "\n");
    uiPrintfEx(trUNMASKABLE, " ************************************************\n");
    uiPrintfEx(trUNMASKABLE, " *  OTA Upgrade Server v1.0.2d                  *\n");
    uiPrintfEx(trUNMASKABLE, " ************************************************\n");
    uiPrintfEx(trUNMASKABLE, "\n");
    ...
    uint16 len, uint8 *pMsg )
{
  ...
  switch ( cmdID )
  {
    ...
    case ZSTACK_CMD_IDS__APP_CNF_COMMISSIONING_NOTIFICATION:
      {
        AppCnfCommissioningNotification *pCommNoti;
        pCommNoti = app_cnf_commissioning_notification__unpack( NULL, len, pMsg );
        if ( pCommNoti )
        {
          processNwkCommissioningNoti( pCommNoti );
          app_cnf_commissioning_notification__free_unpacked( pCommNoti, NULL );
        }
      }
      break;
 ...
 *
 * @brief       Process incoming App message
 *
 * @param       Msg of the type AppCnfCommissioningNotification 
 *
 * @return      none
 *
 ******************************************************************************/
static void processNwkCommissioningNoti( AppCnfCommissioningNotification *pCommNotification )
{
  uiPrintf( "\n\nCommissioning Notification: Status:%d\n\n", pCommNotification->status );
  uiPrintf( "\n\nCommissioning Notification: Commissioning Mode:%d\n\n", pCommNotification->commmode );
  uiPrintf( "\n\nCommissioning Notification: Remiaining Commissioning Mode:%d\n\n", pCommNotification->remainingcommmode );
  sendNwkInfoReadReq();
}
