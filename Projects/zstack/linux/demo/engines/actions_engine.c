...
void act_process_set_color_temp_cnf(pkt_buf_t * pkt, zb_addr_t * addr)
{
  GwZigbeeGenericCnf *msg = NULL;
  ...
  UI_PRINT_LOG("act_process_set_color_temp_cnf: Received ZIGBEE_GENERIC_CNF");
  ...
  if (msg)
  {
    if (msg->status == GW_STATUS_T__STATUS_SUCCESS)
    {
      UI_PRINT_LOG("act_process_set_color_temp_cnf:  Status SUCCESS.");


      if (addr->ieee_addr != 0)
      {
        cluster_id = ZCL_CLUSTER_ID_LIGHTING_COLOR_CONTROL;
        attribute_list[0] = ATTRID_LIGHTING_COLOR_CONTROL_COLOR_TEMPERATURE;
        sr_register_attribute_read_request(msg->sequencenumber,addr, cluster_id, attribute_list, 2);
      }
    }
    else
    {
      UI_PRINT_LOG("act_process_set_color_temp_cnf:  Status FAILURE.");
    }
  ...
  {
    UI_PRINT_LOG("act_process_set_color_temp_cnf: Error Could not unpack msg.");
  }
}
...
void act_set_color_temperature(zb_addr_t * addr, uint16_t colortemperature)
{
  pkt_buf_t * pkt = NULL;
  uint8_t len = 0;
  g_addr = *addr;
  DevSetColorTempReq msg = DEV_SET_COLOR_TEMP_REQ__INIT;
  GwAddressStructT dstaddr;

  si_compose_address(addr, &dstaddr);

  msg.dstaddress = &dstaddr;
  msg.temperaturevalue = colortemperature;

  len = dev_set_color_temp_req__get_packed_size(&msg);
  pkt = malloc(sizeof(pkt_buf_hdr_t) + len);
  ...
  if (pkt)
  {
    pkt->header.len = len;
    pkt->header.subsystem = Z_STACK_GW_SYS_ID_T__RPC_SYS_PB_GW;
    pkt->header.cmd_id = GW_CMD_ID_T__DEV_SET_COLOR_TEMP_REQ ;

    dev_set_color_temp_req__pack(&msg, pkt->packed_protobuf_packet);

    if (si_send_packet(pkt, (confirmation_processing_cb_t)&act_process_set_color_temp_cnf, &g_addr) != 0)
    {
      UI_PRINT_LOG("act_set_color_temp: Error: Could not send msg.");
    }
  ...
  {
    UI_PRINT_LOG("act_set_color_temp: Error: Could not unpack msg.");
  }
}
