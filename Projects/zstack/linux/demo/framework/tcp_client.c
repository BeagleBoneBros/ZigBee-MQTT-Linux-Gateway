...
int tcp_new_server_connection(server_details_t * server_details, char * hostname, u_short port, server_incoming_data_handler_t server_incoming_data_handler, char * name, uint8_t layerNumber, server_connected_disconnected_handler_t server_connected_disconnected_handler)
{
	...
	server_details->layerNumber = layerNumber;
...
int tcp_connect_to_server(server_details_t * server_details)
{
	...
	else
	{
		if ((server_details->fd_index = polling_define_poll_fd(fd, POLLIN, (event_handler_cb_t)tcp_socket_event_handler, server_details)) != -1)
		{
			...
			UI_PRINT_LOG("about to send");
			tcp_send_packet(server_details, tmpstr, strlen(APP_NAME) + 5); //not checking for success since not crucial for normal operation (used only for traces)
	...
	else
	{
		while (remaining_len > 0)
		{
			...
			else
			{
				...
				//Remove this condition if the identify timer spam on the server log and ui console aren't an issue.
				if(!(pkt_ptr->header.cmd_id==GW_CMD_ID_T__GW_WRITE_DEVICE_ATTRIBUTE_RSP_IND))
				{
				...
				visualization_args.remoteLN = server_details->layerNumber;
				visualization_args.remoteName = server_details->name;
				visualization_args.directionSend = false;
				visualization_args.max_layers = INVALID_LAYER_NUMBER;
				//trace_print_buf(pkt_ptr, pkt_ptr->header.len + sizeof(pkt_ptr->header), "[recv] ", &visualization_args);

				//Remove this condition if the identify timer spam on the server log and ui console aren't an issue.
				if(!(pkt_ptr->header.cmd_id==GW_CMD_ID_T__GW_WRITE_DEVICE_ATTRIBUTE_RSP_IND))
				{
					print_pb_msg(pkt_ptr->header.subsystem, pkt_ptr->header.cmd_id, pkt_ptr->packed_protobuf_packet, pkt_ptr->header.len, 0, "", false, &visualization_args, sizeof(pkt_buf_hdr_t), NULL);
				}          
