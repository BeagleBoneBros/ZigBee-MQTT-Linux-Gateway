...
static void nwkMgrDb_FreeDeviceInfoSubstructures( sNwkMgrDb_DeviceInfo_t *pDeviceInfo )
{
  ...
  uiPrintfEx(trINFO, "nwkMgrDb_FreeDeviceInfoSubstructures: Entered pDeviceInfo=0x%x\n", pDeviceInfo);
  ...
  uiPrintfEx(trINFO, "nwkMgrDb_FreeDeviceInfoSubstructures: Exit\n");
}
...
void nwkMgrDb_FreeDeviceInfo( sNwkMgrDb_DeviceInfo_t *pDeviceInfo )
{
  ...
  uiPrintfEx(trINFO, "nwkMgrDb_FreeDeviceInfo: Entered pDeviceInfo=0x%x\n", pDeviceInfo);
...
/**************************************************************************************************
 *
 * @fn      nwkMgrDb_SetDevInfo
 *
 * @brief   Set the device info, based on ieeeAddr
 *
 * @param   ieeeAddr - address of device
 ...
 *
 **************************************************************************************************/
bool nwkMgrDb_SetDevInfo( uint64_t ieeeAddr, uint8 capInfo, uint32_t devAddr)
{
  // SJS_119, fix for Remove Device
  bool worked;
  char szRecord[500];	// allows for maximum sized records
  ...
  // update device shortAddress
  pDeviceInfo->nwkAddr = devAddr;
