 ...
 */
bool gwServices_PollControlStart( afAddrType_t *pSrcAddr, GwAddressStructT * srcAddress )
{  
  ZStatus_t status;
  gwServices_PollControl_PacketPending_t *pEntry;
  
  // Get device information
  pEntry = gwServices_GetPacketQueueEntry( srcAddress->ieeeaddr );
  ...
  status = zclPollControl_Send_CheckInRsp( srvParseGwEndpointId(srcAddress), pSrcAddr,
  ...
  if ( status == ZSuccess )
  {
    ...
    zclPollControl_Send_SetLongPollInterval( srvParseGwEndpointId(srcAddress), pSrcAddr,
                                             giLongPollInterval,
                                             FALSE, zcl_TransID );
                                                              
    // Set Short Poll Interval, assume success
    zclPollControl_Send_SetShortPollInterval( srvParseGwEndpointId(srcAddress), pSrcAddr,
                                              giShortPollInterval,
                                              FALSE, zcl_TransID );
