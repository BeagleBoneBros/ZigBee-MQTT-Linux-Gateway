...
typedef struct
{
  uint8 status;
  uint8 idx;
  ProxyTableEntryFormat_t entryStruct;
}gp_ProxyTblUpdateStatus;
...
extern uint8 gpProxyAliasConflictTimeout;
...
extern uint8 zcl_nv_actions( uint8 action, uint16 id, uint16 index, uint16 len, void *buf );

/*
 * @brief   Handler for the ZCL GreenPower Cluster when
 *          it received an Gp Notification Command for this application.
 */
extern void zclGp_GpPairingCommand( zclGpPairing_t *pCmd );

/*
 * @brief   Handler for the ZCL GreenPower Cluster when
 *          it received a Gp proxy table req.
 */
extern void zclGp_GpProxyTableReq( zclGpProxyTableRequest_t *pCmd );

/*
 * @brief   Callback from the ZCL GreenPower Cluster Library when
 *          it received a Gp Response command. (Ref A.3.5.2.1)
 */
extern void zclGp_GpProxyCommissioningMode(zclGpProxyCommissioningMode_t* pCmd);

/*
 * @brief   Handler for the ZCL GreenPower Cluster when
 *          it received a Gp Commissioning Mode command.
 */
extern void zclGp_GpResponseCommand(zclGpResponse_t* pCmd);
...
/*
 * @brief       Handle GP Attributes Callback
 */
...
/*
 * @brief       Copy the a RAM structure to proxy table entry
 */
extern uint8 gp_copyProxyTblStruct( ProxyTableEntryFormat_t *pEntryStruct, uint8* pEntry );

/*
 * @brief       Copy the proxy table entry to ram
 */
extern uint8 gp_copyProxyTblArray( ProxyTableEntryFormat_t *pEntryStruct, uint8* pEntry );
