...
#define ATTRID_CLUSTER_REV								  0xFFFD   
...
// Format of Lightweight sink address
typedef struct
{
  uint8* sinkIEEE[Z_EXTADDR_LEN];
  uint16 sinkNwkAddr;
} LightweightSinkAddr_t;

// Format of Sink group list
typedef struct
{
  uint16 sinkGroup;
  uint16 alias;
...
typedef struct
{
  ...
  LightweightSinkAddr_t FirstLightweightSinkAddr;
  LightweightSinkAddr_t SecondLightweightSinkAddr;
  uint8                 SinkGroupEntriesBit;
  sinkGroupAddr_t       FirstSinkGroupAddr;
  sinkGroupAddr_t       SecondSinkGroupAddr;
...
extern ProxyTableEntryFormat_t proxyTableEntry[GPP_MAX_PROXY_TABLE_ENTRIES];
/*********************************************************************
 * FUNCTION MACROS
 */
 ...
 */
extern ZStatus_t zclGp_HdlIncoming( zclIncoming_t *pInMsg );
